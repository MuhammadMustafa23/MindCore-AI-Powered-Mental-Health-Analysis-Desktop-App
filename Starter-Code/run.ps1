...
function Ensure-Venv {
    if (-not (Test-Path (Join-Path $ROOT $PYTHON))) {
        ...
        & python -m venv (Join-Path $ROOT $VENV_DIR)
...
function Ensure-Deps {
    ...
    $torchCheck = & (Join-Path $ROOT $PYTHON) -c "import torch, transformers, flask" 2>&1
    if ($LASTEXITCODE -ne 0) {
        ...
        & (Join-Path $ROOT $PIP) install torch --index-url https://download.pytorch.org/whl/cpu --quiet

        # Install rest of requirements
        Write-Host ">>> Installing remaining requirements..." -ForegroundColor Cyan
        & (Join-Path $ROOT $PIP) install -r (Join-Path $ROOT $API_DIR "requirements.txt") --quiet
...
function Start-Api {
    ...
    $script:flask = Start-Process -FilePath (Join-Path $ROOT $PYTHON) `
        -ArgumentList "app.py" `
        -WorkingDirectory (Join-Path $ROOT $API_DIR) `
...
function Start-App {
    ...
    Push-Location (Join-Path $ROOT $APP_DIR)
    mvn javafx:run -q
    Pop-Location
}
...
switch ($Command) {
    "run" {
        Start-Api
        Write-Host ">>> Waiting 10s for model to load..." -ForegroundColor Yellow
    ...
    "api" {
        ...
        & (Join-Path $ROOT $PYTHON) (Join-Path $ROOT $API_DIR "app.py")
    }
    "app" {
        Start-App
    }
